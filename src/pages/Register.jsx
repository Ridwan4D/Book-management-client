...
const Register = () => {
  ...
  const handleSignUp = (e) => {
    ...
    if (
      ...
      setRegisterErr("Use minimum 1 upper & 1 lower case in password");
      return;
    }
    if (!/^(?=.*[A-Z]).+$/.test(password)) {
      setRegisterErr("Use minimum 1 upper case in password");
      return;
    }
    if (!/^(?=.*[a-z]).+$/.test(password)) {
      setRegisterErr("Use minimum 1 lower case in password");
      return;
    }
    ...
    registerUser(email, password)
      ...
      .catch((error) => {
        ...
        if (error.message == "Firebase: Error (auth/email-already-in-use).") {
          setRegisterErr("Email already in use");
          return;
        }
        ...
        <div className="w-full bg-gray-100 lg:w-1/2 flex items-center justify-center">
          <div className="max-w-md w-full p-6">
            ...
            <form onSubmit={handleSignUp} className="space-y-4">
              ...
              {registerErr && <p className="text-sm text-red-600">{registerErr}</p>}
              <div>
                <button
                  type="submit"
